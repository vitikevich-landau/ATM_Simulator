...
struct SceneActorView {
    ...
    // Болтовня в очереди: пузырёк речи над головой ГОВОРЯЩЕГО (0 — молчит).
    // Глиф выбирает презентер (ActorAnim::chatBubble — · ○ ●, все из белого
    // списка); нервные в разговор не допускаются, с «!» не конфликтует.
    char32_t chatBubble = 0;
};
