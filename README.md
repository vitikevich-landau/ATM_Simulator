...
обратно. В блоке статистики видно `Обслужено N / T` — сколько обслужено из
**общего плана прогона** (`clients.count`), чтобы во время прогона было понятно,
близко ли конец. Команда `restart` перезапускает прогон, **перечитывая конфиг с
диска**: правки файла (валюта, число клиентов, тайминги) применяются без
перезапуска программы (при неизменном `random_seed` прогон воспроизводится
точно — поменяйте seed для нового случайного сценария). Если stdout перенаправлен
в файл/пайп, live-режим автоматически отключается (командный режим, без ANSI).
...
- Денежные суммы хранятся в **целочисленных** минимальных единицах валюты
  (копейки/центы), а не в `float`/`double` (§4.2 ТЗ). При выводе форматируются
  под валюту `atm.currency` — символ и его позиция, группировка разрядов,
  десятичный знак: EUR → «500 000.00 €», USD → «$500,000.00», RUB →
  «500 000.00 ₽», неизвестный код → «500 000.00 XXX». Любое правило можно
  переопределить в `atm.currency_format` (см. `config/default_config.json`).
